...
public class SigningJobTests {
    ...
    void testEnd2EndHtmlTransformationEncoding() throws IOException, RequestValidationException {
        var ssParams = new VersionedSigningParameters(
                SignatureForm.XAdES,
        ...
        var xdcParameters = new XDCParameters(null, null, identifier, containerXmlns, false, schema, "application/xml;base64", null, transformation, null, null, null, null);
        var document = new Document("document.xml", content, "application/xml;base64", xdcParameters); 

        var signRequestBody = new VersionedSignRequestBody(document, null, ssParams, null, null);
        var input = signRequestBody.getSigningInput(true);
        var job = SigningJob.fromInput(input);
        try {
            job.initializeVisualizations();
            var visualization = job.getVisualizations().get(0);
            assertInstanceOf(HTMLVisualization.class, visualization);
            var htmlTransformed = ((HTMLVisualization) visualization).getDocument();
            var expected = new String(TestMethodSources.loadContent("transformed.html"), StandardCharsets.UTF_8);
    ...
    @Test
    void testEnd2EndHtmlTransformationEncodingWithoutSchemaMimeType() throws IOException, RequestValidationException {
        var ssParams = new VersionedSigningParameters(
                SignatureForm.XAdES,
                SignatureProfile.BASELINE_B,
                ASiCContainerType.ASiC_E,
        ...
        var xdcParameters = new XDCParameters(null, null, identifier, containerXmlns, false, schema, null, null, transformation, null, null, null, null);
        var document = new Document("document.xml", content, "application/xml;base64", xdcParameters); 
        var signRequestBody = new VersionedSignRequestBody(document, null, ssParams, null, null);
        var input = signRequestBody.getSigningInput(true);
        var job = SigningJob.fromInput(input);
    ...
    void signingPdfAsPadesWithAsicEContainerMustNotThrowClassCastException() throws IOException {
        ...
        var thrown = assertThrows(Throwable.class, () -> job.signWithKey(key, null),
