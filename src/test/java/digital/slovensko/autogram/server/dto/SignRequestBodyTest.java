...
class SignRequestBodyTest {
    ...
    void testValidateXDCWithoutXSD() throws IOException {
        ...
        var document = new LegacyDocument("document.xml", xmlContent);

        var signingParameters = ServerSigningParametersTestFactory.create(
        ...
        var versionedBody = signRequestBody.toVersionedBody();

        Assertions.assertDoesNotThrow(() -> {versionedBody.getSigningInput(true);});
    }
    ...
    void testValidateXDCWithInvalidXmlAgainstXSD() throws IOException {
        ...
        Assertions.assertThrows(XMLValidationException.class, () -> {versionedBody.getSigningInput(true);});
    }
    ...
    void testValidateXDCEmptyXSLT() {
        ...
        Assertions.assertThrows(TransformationParsingErrorException.class, () -> {versionedBody.getSigningInput(true);});
    }
    ...
    void testValidateXDCBase64InvalidXSD() {
        ...
        Assertions.assertThrows(EFormException.class, () -> {versionedBody.getSigningInput(true);});
    }
    ...
    void testAutoLoadsEformByDefaultForAsicePayload() throws IOException {
        ...
        var signRequestBody = new SignRequestBody(new LegacyDocument("file.asice", content), createParametersWithAutoLoadEform(null),
                "application/vnd.etsi.asic-e+zip; base64");

        var versionedBody = signRequestBody.toVersionedBody();
        var signingInput = versionedBody.getSigningInput(true);
        var eFormAttributes = signingInput.getFirstDocument().getEFormAttributes();
    ...
    void testAutoLoadsEformByDefaultForXdcPayload() throws IOException {
        ...
        var signRequestBody = new SignRequestBody(new LegacyDocument("document.xml", content),
    ...
    void testDoesNotAutoLoadEformForAsicePayloadWhenExplicitlyDisabled() throws IOException {
        var content = Base64.getEncoder().encodeToString(
                this.getClass().getResourceAsStream("../../general_agenda.asice").readAllBytes());
        var signRequestBody = new SignRequestBody(new LegacyDocument("file.asice", content), createParametersWithAutoLoadEform(false),
                "application/vnd.etsi.asic-e+zip; base64");
    ...
    private static ServerSigningParameters createParametersWithLevelAndContainer(
            ServerSigningParameters.LocalSignatureLevel level, ASiCContainerType container) {
        return ServerSigningParametersTestFactory.create(level, container, null, null, null, DigestAlgorithm.SHA256,
                null, null, null, null, null, null, null, false, null, null, false, null, null, null, null, null, null);
    }

    @Test
    void testMissingDocumentIsReportedAsMissingField() {
        var signRequestBody = new SignRequestBody(null,
                createParametersWithLevelAndContainer(ServerSigningParameters.LocalSignatureLevel.XAdES_BASELINE_B, null),
                "application/xml");

        var exception = Assertions.assertThrows(RequestValidationException.class, signRequestBody::toVersionedBody);

        Assertions.assertEquals("Document is required",
                exception.getSubheading(SupportedLanguage.ENGLISH.loadResources()));
    }

    @Test
    void testPAdESWithNonPdfDocumentIsRejected() throws IOException {
        var content = new String(this.getClass().getResourceAsStream("../../general_agenda.xml").readAllBytes());
        var signRequestBody = new SignRequestBody(new LegacyDocument("document.xml", content),
                createParametersWithLevelAndContainer(ServerSigningParameters.LocalSignatureLevel.PAdES_BASELINE_B, null),
                "application/xml");

        var versionedBody = signRequestBody.toVersionedBody();

        var exception = Assertions.assertThrows(RequestValidationException.class,
                () -> versionedBody.getSigningInput(true));

        Assertions.assertEquals("PayloadMimeType and Parameters.Level mismatch",
                exception.getSubheading(SupportedLanguage.ENGLISH.loadResources()));
    }

    @Test
    void testPAdESWithContainerIsRejected() throws IOException {
        var content = Base64.getEncoder().encodeToString(
                this.getClass().getResourceAsStream("../../sample.pdf").readAllBytes());
        var signRequestBody = new SignRequestBody(new LegacyDocument("document.pdf", content),
                createParametersWithLevelAndContainer(ServerSigningParameters.LocalSignatureLevel.PAdES_BASELINE_B,
        ...
        Assertions.assertEquals("Parameters.Container is not supported for PAdES",
                exception.getSubheading(SupportedLanguage.ENGLISH.loadResources()));
    }

    @Test
    void testPAdESWithPdfDocumentIsAccepted() throws IOException {
        var content = Base64.getEncoder().encodeToString(
                this.getClass().getResourceAsStream("../../sample.pdf").readAllBytes());
        var signRequestBody = new SignRequestBody(new LegacyDocument("document.pdf", content),
                createParametersWithLevelAndContainer(ServerSigningParameters.LocalSignatureLevel.PAdES_BASELINE_B, null),
        ...
        Assertions.assertDoesNotThrow(() -> signRequestBody.toVersionedBody().getSigningInput(true));
    }
}
