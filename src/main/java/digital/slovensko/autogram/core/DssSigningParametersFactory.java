...
public abstract class DssSigningParametersFactory {
    public static AbstractSignatureParameters<?> createSignatureParameters(SigningInput input, SigningKey key) {
        var isContainer = input.getParameters().getContainer() != null;
        var signatureParameters = switch (input.getParameters().getSignatureForm()) {
            case XAdES -> isContainer ? createASiCWithXAdESSignatureParameters(input) : createXAdESSignatureParameters(input);
            case CAdES -> isContainer ? createASiCWithCAdESSignatureParameters(input) : createCAdESSignatureParameters(input);
            case PAdES -> createPAdESSignatureParameters(input);
