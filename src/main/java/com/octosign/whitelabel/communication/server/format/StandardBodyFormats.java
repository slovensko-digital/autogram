...
public enum StandardBodyFormats implements BodyFormat {
    JSON {
        ...
        static class ExceptionSerializer implements JsonSerializer<IntegrationException> {
            @Override
            public JsonElement serialize(IntegrationException src,
                    Type type,
        ...
        static class SignRequestDeserializer implements JsonDeserializer<SignRequest> {
            @Override
            public SignRequest deserialize(JsonElement jElement,
                    Type type,
                    JsonDeserializationContext context) {
                JsonObject signRequest = jElement.getAsJsonObject();

                String hmac = getOptional("hmac", signRequest);
                MimeType payloadType = context.deserialize(signRequest.get("payloadMimeType"),
                        MimeType.class);
                ...
                MimeType transformationOutputMimeType = context.deserialize(params.get(
                        "transformationOutputMimeType"), MimeType.class);
                ...
                DigestAlgorithm digestAlgorithm = context.deserialize(params.get("digestAlgorithm"),
                        DigestAlgorithm.class);
                Boolean en319132 = isNullValue("en319132", params) ? null : params.get("en319132").getAsBoolean();
                CanonicalizationMethod infoCanonicalization = context.deserialize(params.get(
                        "infoCanonicalization"), CanonicalizationMethod.class);
                CanonicalizationMethod propertiesCanonicalization = context.deserialize(params.get(
                        "propertiesCanonicalization"), CanonicalizationMethod.class);
                CanonicalizationMethod keyInfoCanonicalization = context.deserialize(params.get(
                        "keyInfoCanonicalization"), CanonicalizationMethod.class);
