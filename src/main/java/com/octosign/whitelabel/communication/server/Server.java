...
import com.octosign.whitelabel.ui.SignerException;
import com.octosign.whitelabel.ui.UserException;
...
public class Server {
    ...
    public Server(String hostname, int port, int initialNonce) throws UserException, IntegrationException {
        try {
            server = HttpServer.create(new InetSocketAddress(hostname, port), 0);
        } catch (BindException e) {
            throw new UserException("error.serverNotCreated", e);
        } catch (IOException e) {
            throw new IntegrationException("error.serverNotCreated", e);
        }
