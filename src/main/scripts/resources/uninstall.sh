...
# security remove-trusted-cert "$TLS_DIR/octosign-cert.pem"
# security delete-certificate -c "localhost.arcicode.com" "$HOME/Library/Keychains/login.keychain-db"
security delete-certificate -Z $(openssl x509 -in "$TLS_DIR/octosign-cert.pem" -outform DER | shasum -a 1 ) "$HOME/Library/Keychains/login.keychain-db"
